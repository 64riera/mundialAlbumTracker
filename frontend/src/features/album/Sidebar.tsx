...
function ConfederationGroup({
  ...
  return (
    <div className="mb-1">
      <button
        onClick={() => setOpen((v) => !v)}
        className="w-full flex items-center gap-2 px-3 py-2 text-xs font-semibold text-slate-400 hover:text-slate-600 uppercase tracking-wider transition-colors"
      >
              ...
              {({ isActive }) => (
                <>
                  ...
                  <div className="flex flex-col items-end gap-1 shrink-0">
                    <span className={cn("text-[10px]", isActive ? "text-brand-200" : "text-slate-400")}>
                      {section.percentage}%
                    </span>
                    <div className="w-12">
                      <ProgressBar
  ...
  return (
    <nav className="h-full overflow-y-auto py-3 space-y-1">
      ...
      <div className="px-3 pb-1">
        <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider px-0 mb-2">
          Álbum
        </p>
