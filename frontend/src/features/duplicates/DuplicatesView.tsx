    ...
    <div className="p-4 md:p-6 space-y-5">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-slate-800">
          🔄 Duplicadas{" "}
          <span className="text-lg text-slate-400 font-normal">({duplicates?.length ?? 0})</span>
        </h1>
        {(duplicates?.length ?? 0) > 0 && (
          <button
            onClick={handleCopyList}
            className="flex items-center gap-2 text-sm text-slate-600 hover:text-brand-600 border border-slate-200 hover:border-brand-300 rounded-lg px-3 py-2 transition-colors"
          >
            <Copy size={14} />
            Copiar lista
          </button>
          ...
          {duplicates.map((s) => (
            <div key={s.id} className="flex items-center gap-3 px-4 py-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-400">{s.section?.name}</p>
              </div>
